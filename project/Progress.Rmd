...
TheLevels = c("Location.1", "Location.2", "Location.3", "Location.4", "Location.5")

for (level in TheLevels){
    for (i in levels(data[level])){
...
for (i in 1:nrow(mydata)){
  ...
  for (j in 1:nrow(data)){
    temp = data[j,]
    if ((temp$Location.1 == location_levels[i]) | 
        (temp$Location.2 == location_levels[i]) | 
        (temp$Location.3 == location_levels[i]) |
        (temp$Location.4 == location_levels[i]) | 
        (temp$Location.5 == location_levels[i])){
