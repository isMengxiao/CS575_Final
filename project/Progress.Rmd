...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    ggmap(myMap)+geom_point(aes(x=longitude, y=latitude, size=sets, colour = cut(sets, c(0, 10, 20, 60))), data = newdata, alpha=0.8)+scale_colour_manual(name = "sets",
                     values = c("green", "red", "black"),
                     labels = c("<= 10", "10 < sets <= 20", "> 20"))
  }) 
